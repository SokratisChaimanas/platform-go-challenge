...
func (_q *AssetQuery) QueryFavourites() *FavouriteQuery {
	query := (&FavouriteClient{config: _q.config}).Query()
...
func (_q *AssetQuery) Clone() *AssetQuery {
	...
	return &AssetQuery{
		...
		withFavourites: _q.withFavourites.Clone(),
...
func (_q *AssetQuery) WithFavourites(opts ...func(*FavouriteQuery)) *AssetQuery {
	query := (&FavouriteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withFavourites = query
	return _q
}
...
func (_q *AssetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Asset, error) {
	...
	if query := _q.withFavourites; query != nil {
		if err := _q.loadFavourites(ctx, query, nodes,
			func(n *Asset) { n.Edges.Favourites = []*Favourite{} },
			func(n *Asset, e *Favourite) { n.Edges.Favourites = append(n.Edges.Favourites, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AssetQuery) loadFavourites(ctx context.Context, query *FavouriteQuery, nodes []*Asset, init func(*Asset), assign func(*Asset, *Favourite)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(favourite.FieldAssetID)
	}
	query.Where(predicate.Favourite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(asset.FavouritesColumn), fks...))
	}))
